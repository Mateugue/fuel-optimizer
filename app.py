...
st.caption("V0.4.1 — géocodage + itinéraire + prix réels des carburants")

st.header("1. Véhicule")
c1, c2 = st.columns(2)
with c1:
    fuel_type = st.selectbox(
        "Carburant", ["SP95-E10", "SP98", "SP95", "E85", "Diesel", "GPLc"]
    )
    tank = st.number_input("Capacité du réservoir (L)", 20.0, 150.0, 53.0, 1.0)
    consumption = st.number_input(
        "Consommation moyenne (L/100 km)", 2.0, 30.0, 7.0, 0.1
    )

with c2:
    mode = st.radio(
        "Indication du véhicule",
        ["Autonomie restante (km)", "Niveau de carburant (%)"],
    )
    if mode.startswith("Autonomie"):
        range_km = st.number_input(
            "Autonomie affichée (km)", 1.0, 2000.0, 280.0, 10.0
        )
        fuel_remaining = range_km * consumption / 100.0
    else:
        pct = st.slider("Niveau de carburant (%)", 1, 100, 40)
        fuel_remaining = tank * pct / 100.0
        range_km = fuel_remaining / consumption * 100.0

    fuel_remaining = min(fuel_remaining, tank)
    range_km = fuel_remaining / consumption * 100.0
    st.caption(
        f"Carburant estimé : {fuel_remaining:.1f} L — autonomie : {range_km:.0f} km"
    )
...
safety = st.number_input(
    "Marge de sécurité minimale (km)", 0.0, 200.0, 20.0, 5.0
)
...
if st.button("Analyser le trajet", type="primary"):
    try:
        ...
        normal_km = normal["distance"] / 1000.0
        normal_min = normal["duration"] / 60.0
        ...
        with st.spinner("Recherche des stations réelles..."):
            raw = stations_near_route(
                geometry, fuel_type, max_from_route, 12
            )
            ...
            try:
                ...
                stations.append(
                    Station(
                        ...
                        max(r["distance"] / 1000.0 - normal_km, 0.0),
                        max(r["duration"] / 60.0 - normal_min, 0.0),
        ...
        # Reference station = the feasible station with the smallest complete detour.
        safety_l = safety * consumption / 100.0
        ...
        for s in stations:
            ...
            needed = s.distance_to_station_km * consumption / 100.0
            remaining = fuel_remaining - needed
            if remaining < safety_l:
                continue

            fuel_after_refuel = tank
            needed_destination = s.station_to_destination_km * consumption / 100.0
        ...
        if not results:
            st.warning(
                "Aucune station ne respecte toutes les contraintes et le seuil d'économie nette."
            )
        else:
            best = results[0]
            s = best["station"]
            ...
            st.write(
                f"Carburant à l'arrivée à la station : "
            ...
            st.write(
                f"Carburant estimé à l'arrivée à destination : "
                f"{best['fuel_at_destination']:.2f} L"
            )
            st.write(
                f"Autonomie estimée à l'arrivée : "
            ...
            for item in results[1:]:
                s = item["station"]
                st.write(
                    f"**{s.name}** — {s.price_per_l:.3f} €/L — "
                    f"+{s.detour_minutes:.0f} min — économie {item['net_saving']:.2f} € — "
